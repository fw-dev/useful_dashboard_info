...
collect_statement = "select u.name, u.id, i.last_check_in, now()::date - i.last_check_in::date how_old, " \
        "dc.filewave_model_number, i.id from admin.user_clone_group u, public.generic_genericclient i, " \
        "public.generic_genericdesktopclient dc where u.id = i.filewave_id and i.id = dc.genericclient_ptr_id"
